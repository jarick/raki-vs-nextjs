    ...
    fn json_to_rsc_element(
        ...
        if let Some(obj) = json.as_object() {
            ...
            if is_lazy {
                ...
                let marker = match boundary_lazy_refs.get(promise_id) {
                    Some(&promise_row_id) => format!("${:x}", promise_row_id),
                    None => format!("%{}%", promise_id),
                };

                return Ok(serde_json::Value::String(marker));
            }
