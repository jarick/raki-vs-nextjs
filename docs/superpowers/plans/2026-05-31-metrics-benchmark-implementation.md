...
| TTFB–TTLB delta | ~14ms (streaming) | ~0ms (buffered) |

Rari streams the RSC response: the first byte arrives immediately, remaining bytes arrive as async components resolve. Next.js buffers the entire response before sending — TTFB ≈ TTLB.
```
...
Rari shows linear growth up to 200 connections, then plateaus. Next.js saturates at 50 connections — event loop cannot keep up.
```
...
Rari starts with cache warmup (1 route pre-rendered) — all 3.5M requests are served from cache. Next.js with `force-dynamic` renders every request.
```
...
CPU·ms per request: Rari spends 7 microseconds of CPU per request, Next.js — 2 milliseconds. Difference of ~300x.
```
...
```markdown
| Stage | Rari (tracing) | Next.js (built-in) |
|-------|----------------|-------------------|
